...
class GeneticAlgorithm(object):
    ...
    def weight_average_crossover(self, parent_1, parent_2):
        ...
        for i in range(crossover_point):
        	...
        	offspring_1[i] = alpha*parent_1[i] + (1-alpha)*parent_2[i]

        for i in range(crossover_point,size):
        	alpha=random.uniform(0, 1)
        	offspring_2[i] = alpha*parent_1[i] + (1-alpha)*parent_2[i]
    ...
    def GAPipeLine(self, individuals, pk, ck):
        ...
        chosen_1 = self.selBest(individuals, (pk))
       # chosen_1.append(champion[0])
        chosen_2 = self.selRandom(chosen_1, ck + 2)
...
class NeuralNetwork(object):
    ...
    def run(self, input_vector):
        ...
        output_vector = np.tanh(output_vector) * 3 #ERROR->Still gives 6 as maximum output

        return [item for sublist in output_vector for item in sublist]


class RecurrentNeuralNetwork(object):
	#FIX BIAS NODE to work
    def __init__(self, no_of_in_nodes, no_of_out_nodes, no_of_hidden_nodes, bias=None):
        self.no_of_in_nodes = no_of_in_nodes
        self.no_of_out_nodes = no_of_out_nodes
        self.no_of_hidden_nodes = no_of_hidden_nodes
        ...
        self.weights_recurrent_out = np.zeros(shape=(self.no_of_recurrent_nodes, self.no_of_out_nodes))
    ...
    def create_weight_matrices(self):
        ...
        X = self.truncated_normal(mean=0, sd=1, low=-rad, upp=rad)
        self.weights_recurrent_out = X.rvs((self.no_of_out_nodes, self.no_of_out_nodes))
    ...
    def create_chromosome(self):
        wih = self.weights_in_hidden.flatten()
        woh = self.weights_hidden_out.flatten()
        wro = self.weights_recurrent_out.flatten()
        chromosome = np.concatenate((wih, woh, wro), axis=None)
        return chromosome
    ...
    def update_weights(self, chromosome):
        ...
        self.weights_hidden_out = chromosome[ih_weight_no:(ih_weight_no + ho_weight_no)].reshape(
            self.no_of_out_nodes,
            self.no_of_hidden_nodes)
        self.weights_recurrent_out = chromosome[(ih_weight_no + ho_weight_no):].reshape(
            self.no_of_out_nodes,
    ...
    def run(self, input_vector, prev_output_vector):  # ONLY THIS REMAINS
        ...
        recurrent_vector = np.dot(self.weights_recurrent_out, prev_output_vector)
        ...
        output_vector = np.tanh(output_vector) * self.maxVel
...
class Dust(object):
    ...
    def __init__(self, speckNumber, maxX, maxY, radius, win):
        self.specks = []
...
class Robot(object):
    ...
    def fitness_function(self):
        ...
        if minDist <= 3:
         ...
         #   self.collisionScore += 1  # total dt that the robot has been colliding
          #  col = 1
        dustFactor = np.tanh(self.dustEaten-self.prev_DustEaten)
        self.fitnessScore += abs(self.velocity) * abs(1 - math.sqrt(abs(self.velL - self.velR) / (2*self.velMax))) * sensorFactor * dustFactor
        #self.fitnessScore = self.dustEaten
...
def redrawGameWindow(win, robot, walls, dust):
    win.fill((WHITE_2))
    dust.draw()
    for wall in walls:
        wall.draw(win)
